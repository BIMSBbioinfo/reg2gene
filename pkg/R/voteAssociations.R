...
#' @param qval.cutoff q-value cutoff that will be used to filter elements in the
...
#'                       package = "reg2gene")
#'
#' voted=voteAssociations(assoc, qval.cutoff=0.05,vote.threshold=0.5)
#'
#' # get all the vote counts
#' voted=voteAssociations(assoc, qval.cutoff=0.05,vote.threshold=0)
#' }
#' @export
#' @author Altuna Akalin
#'
#'
voteAssociations<-function(associations, qval.cutoff=0.05,vote.threshold=0.5){
  ...
  dtl=assocTable2(associations,qval=qval.cutoff)
  ...
  gr[gr$votes >= (vote.threshold*ncol(q)),]
}
...
assocTable2<-function(grlist,qval){
  ...
  dt.list=lapply(grlist,function(x,qval){
    ...
    data.table(ky=paste(as.character(x),x$name,x$name2,
               ...
               association=1,key="ky")},qval=qval
